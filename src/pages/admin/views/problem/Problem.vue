    ...
    <Panel :title="title">
      <el-form ref="form" :model="problem" :rules="rules" label-position="top" label-width="70px">
        ...
        <el-row :gutter="20">
          <!--          分类s-->
          <el-col :span="20">
            <el-form-item :label="$t('m.Classify')">
              <el-checkbox-group v-model="value">
                <el-checkbox v-for="item in classifyList" :label="item.name" :key="item.id" @change="openSubclass"></el-checkbox>
              </el-checkbox-group>
              <el-checkbox-group v-model="valueTwo" v-if="subclassState">
                <el-checkbox v-for="lang in subclassList" :key="lang.id" :label="lang.name"></el-checkbox>
              </el-checkbox-group>
            </el-form-item>
          </el-col>
          <!--          分类end-->
...
<script>
  ...
  export default {
    ...
    mounted () {
      ...
      api.getProblemClassifyList().then(res => {
    ...
    watch: {
      ...
      // 'value' (newVal) {
      //   // use deep copy to avoid infinite loop
      //   let classifys = JSON.parse(JSON.stringify(newVal)).sort()
      //   this.problem.classifys = classifys
      //   console.log(this.problem.classifys)
      // },
      // 添加子类
      'valueTwo' (newVal) {
        // use deep copy to avoid infinite loop
        let classifys = JSON.parse(JSON.stringify(newVal)).sort()
        this.problem.classify = classifys
      },
    ...
    methods: {
      ...
      querySearchClassify (queryString, cb) {
        api.getProblemClassifyList().then(res => {
          let classifyList = []
          for (let classify of res.data.data) {
            classifyList.push({value: classify.name})
          }
          cb(classifyList)
        }).catch(() => {
        })
      ...
      // addClassify () {
      //   let inputValue = this.classifyInput
      //   if (inputValue) {
      //     this.problem.classifys.push(inputValue)
      //   }
      ...
      // closeClassify (classify) {
      //   this.problem.classifys.splice(this.problem.classifys.indexOf(classify), 1)
      // },
      addSample () {
      ...
      submit () {
        ...
        // if (!this.problem.classifys.length) {
        //   this.error.tags = 'Please add at least one classify'
        //   this.$error(this.error.tags)
        //   return
        // }
        if (this.problem.spj) {
      ...
      // 筛选子类
      openSubclass () {
        this.subclassState = true
        let obj = {}
        if (this.value.length > 1) {
          this.value.splice(0, 1)
        }
        obj = this.classifyList.find((item) => { // 这里的userList就是上面遍历的数据源
          return item.name === this.value[0] // 筛选出匹配数据
        })
        let vId = obj.id
        api.getSubclassListSelect(vId).then(res => {
